...
export default function PayPage() {
  ...
  useEffect(() => {
    ...
    // Pick up where we left off if the MAC matches (prevent cross-user pickup)
    ...
    // 4. Fetch Plans with Tunnel Bypass
    ...
    const handleOnline = () => setStatus(null);
  ...
  useEffect(() => {
    ...
    } else if (countdown === 0 && isWaitingForPin) {
      // Don't kill the session, just stop the timer
    }
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.status === "success") {
        // Handle both Redirect (Paystack) and STK Push (M-Pesa)
  ...
  const pollVerification = async (ref: string) => {
    ...
    // Fast polling (every 2s) for better UX
      ...
      <div style={{
        ...
        {tunnelBlocked ? (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <button onClick={() => window.location.reload()} style={{ display: "block", width: "100%", marginTop: "12px", background: "none", border: "1px solid #e5e7eb", color: "#4b5563", padding: "14px", borderRadius: "12px", fontWeight: "700" }}>
              Reload Page
            </button>
          </div>
        ) : isWaitingForPin ? (
          <div style={{ textAlign: "center", padding: "40px 20px" }}>
            ...
            <button
              onClick={() => { setIsWaitingForPin(false); localStorage.removeItem('active_checkout_ref'); }}
            ...
            <div style={{ marginTop: "24px", textAlign: "center" }}>
                {!showRebind ? (
                    ...
                    <form onSubmit={handleRebind} style={{ borderTop: "1px solid #f3f4f6", paddingTop: "20px" }}>
                        <p style={{ fontSize: "12px", color: "#6b7280", marginBottom: "12px" }}>Enter your Voucher Code or Phone Number used during payment</p>
